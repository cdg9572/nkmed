...
//문진
function funcListMJ($dir, $menu, $tbl, $field, $keyword, $where, $searchQuery, $listNum, $pageNum, $orderByColumn, $orderBy) {
	...
	// 게시물 총개수 셀렉트
	$que = "SELECT COUNT(*) AS total FROM ".$tbl." WHERE midx<>'' ";
	//echo $que;
	$que .= $where;
	...
	// 해당 페이지 셀렉트
	$que = "SELECT * FROM ".$tbl." WHERE midx<>''  ";
	$que .= $where;
	if($field && $keyword) $que .= " AND ".$field." LIKE '%".$keyword."%'";
	$que .= " ORDER BY ";
...
//학생검진
function funcListST($dir, $menu, $tbl, $field, $keyword, $where, $searchQuery, $listNum, $pageNum, $orderByColumn, $orderBy) {
	$numPerPage		=	$listNum ? $listNum : 10;				// 게시판 리스트 출력수
	$pagePerList	=	$pagePerList ? $pagePerList : 10;	// 게시판 페이징 노출수
	$pageNum		=	$pageNum ? $pageNum : 1;
	$orderByColumn	=	$orderByColumn ? $orderByColumn : "reg_date";
	...
	// 게시물 총개수 셀렉트
	$que = "SELECT COUNT(*) AS total FROM ".$tbl." WHERE sidx<>'' ";
	//echo $que;
	$que .= $where;
	if($field && $keyword) $que .= " AND ".$field." LIKE '%".$keyword."%'";
	...
	// 해당 페이지 셀렉트
	$que = "SELECT * FROM ".$tbl." WHERE sidx<>''  ";
	$que .= $where;
	if($field && $keyword) $que .= " AND ".$field." LIKE '%".$keyword."%'";
	$que .= " ORDER BY ";
