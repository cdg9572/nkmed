...
function get_category_desc($m_item, $tr_check =0)
{
	...
	else 
	{
		...
		$sql = "select * from `smart_m_desc` where m_item ='$m_item' ORDER BY reg_date DESC" ;
	//echo "$sql";
		$res = MYSQL_QUERY($sql); 
		$row = MYSQL_FETCH_ARRAY($res);

	//$sql = "<br>$sql";
		if($tr_check == 1 || !$row[m_item_cn])
		{
			$str .= "			
				<div>
		...
		else
		{
			$str .= "			
				<div>
					<h3> $row[m_item_cn]  &nbsp; <i class='fa fa-commenting fa-2x'></i></h3>
					<p> $row[m_desc_cn]</p>
					<p style='height:10 ;'>
				</div>
...
function tr_cn($desc_kr, $tr_check =0) 
{
	...
	{
		$desc_kr = str_replace("Positive","양성", $desc_kr) ;
		$desc_kr = str_replace("Negative","음성", $desc_kr) ;

		$desc_kr = str_replace("Non-reactive","음성", $desc_kr) ;


		if(!$desc_kr) //== 한글이고 검사결과가 없을때 
		{
			$desc_kr = "미실시";
		}
	}
	...
	$sql = "select * from multi_lan where desc_kr =  replace('$desc_kr', ' ', '') ORDER BY reg_date DESC" ;
	...
	if(!$re) 
	{
		$sql = "select * from multi_lan where key_kr = replace('$desc_kr', ' ', '') ORDER BY reg_date DESC" ;
		$res = MYSQL_QUERY($sql) or die(mysql_error()) ;
		$row = mysql_fetch_array($res) ;

		$re2 = nl2br($row[key_cn]) ;
