btn_confirm	//변수 선언 및 초기화
    ...
    function html5Upload() {
    	...
    	sUploadURL= "http://" + domain + '/bbs/smarteditor/popup/quick_photo/FileUploader_html5.php'; 	//upload URL
    ...
    function callAjaxForHTML5 (tempFile, sUploadURL){
		...
		var foldername = opener.parent.document.getElementById('upfolder').value;
		...
		oAjax.header("floderId", foldername);
	...
	function onAjaxError (){
 	...
 	function callFileUploader (){
		...
		oFileUploader = new jindo.FileUploader(jindo.$("uploadInputBox"),{
 			sUrl  : "http://" + domain + '/bbs/smarteditor/popup/quick_photo/FileUploader.php',	//샘플 URL입니다.
 	        sCallback : location.href.replace(/\/[^\/]*$/, '') + '/callback.html',	//업로드 이후에 iframe이 redirect될 콜백페이지의 주소
			htData : { floderId : foldername },
	 ...
	 */
 	...
 	function setPhotoToEditor(oFileInfo){
		if (!!opener && !!opener.nhn && !!opener.nhn.husky && !!opener.nhn.husky.PopUpManager) {
			...
			var FileName = oFileInfo[0]['sFileName'];

//			for (myKey in oFileInfo[0]){
//				alert ("oFileInfo["+myKey +"] = "+oFileInfo[myKey]);
//			}

			opener.parent.insertIMG(FileName);
