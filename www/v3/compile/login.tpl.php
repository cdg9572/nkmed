<?php /* Template_ 2.2.8 2023/07/27 08:55:54 /home/hosting_users/nkmed/www/v3/view/login.tpl 000004224 */ ?>
...
<form name="l_form" id="l_form" method="post">
    ...
    <input type="hidden" name="r_mode" id="r_mode" value="kakao">
    <input type="hidden" name="u" value="M">
	<input type="hidden" name="mb_id" id="r_id">
	<input type="hidden" name="mb_name" id="r_name">
	<input type="hidden" name="mb_mobile" id="r_mobile">
	<input type="hidden" name="mb_birthday" id="r_birthday">
</form>
<style type="text/css">
	#kakao_login:before{
		color: #412e34;
...
<script>Kakao.init('4956ab3b023064a743913019ad22db1d');</script>
<script>
		...
		function getKakaotalkUserProfile(){
			Kakao.API.request({
				...
				success: function(res) {                      
					$("#r_id").val(res.id);					
					$("#r_name").val(res.kakao_account.name);                    
					$("#r_mobile").val(res.kakao_account.phone_number);
					$("r_birthday").val(res.kakao_account.birthday);
					
					$("#l_form").attr("action","/v3/controller/action/login_action.php");
					$("#l_form").submit();
				},
				fail: function(error) {
					console.log(error);
