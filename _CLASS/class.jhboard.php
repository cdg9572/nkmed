...
CLASS JHBoard extends Dbconn
{
	...
	function Board_Main_Data($args = '') {
		...
		if($jb_code=="news") {
			$addQry = "A.jb_code in ('10','140','50','80')";
		}else{
	...
	function Board_List($args = '') {
		...
		$tail = "jb_code=$jb_code&search_key=$search_key&search_keyword=$search_keyword";
        ...
        if($jb_arrtreat != '') {
            $addQry .= "and A.jb_treat in ($jb_arrtreat )";
        }
    ...
    function Board_List_Search($args = '') {
		global $C_Func;
		global $C_ListClass;
        ...
        if($s_search != '') {   			
				$addQry .= " AND (jb_title LIKE '%${s_search}%' || jb_content LIKE '%${s_search}%')";
			}	
