...
CLASS Popup extends Dbconn
{
	...
	function PopUp_Show() {
		...
		$qry = "
			SELECT * FROM tblPopUp WHERE pop_use='Y' AND ('".date('Y-m-d H:i:s')."' BETWEEN pop_start_date AND pop_end_date) and pop_type3 = 'PC' ORDER BY pop_idx ASC
        ";
	...
	function PopUp_Show_Mobile() {
		...
		$qry = "
			SELECT * FROM tblPopUp WHERE pop_use='Y' AND ('".date('Y-m-d H:i:s')."' BETWEEN pop_start_date AND pop_end_date) and pop_type3 = 'mobile' ORDER BY pop_idx desc limit 0,3
		";
		$rst =  $this -> DB -> execSqlList($qry);
		return $rst;
	}	
	...
	function Popup_List ($args = '') {
        ...
        if($pop_type3 != '') {
			$addQry .= " and pop_type3='$pop_type3' ";
		}
